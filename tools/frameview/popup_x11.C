...
/******************************************************************************/
/******************************************************************************/
PopupX11::MyWin::MyWin(Display *d, Window w) {
  disp= d; 
  parent_win= w;
...
void PopupX11::MyWin::resize(int s_x, int s_y) {
...
void PopupX11::MyWin::set_visible(bool flag) {
  visible= flag;
  if (visible) {
    if (!exists)
      create();
...
void PopupX11::MyWin::move(int xx, int yy) {
  x= xx;
  y= yy;
  if (!exists)
    return;
...
/******************************************************************************/
int PopupX11::get_text_width(int len) const {
  return len* char_max_width;
}

void PopupX11::compute_size_for_buttons_window(int & s_x, int & s_y) const {
...
PopupX11::PopupX11(Display *d, Window pw, GC g): subwin(d,pw) {
 disp= d; 
 win= pw; 
 gc= g; 
 ...
 if (fontstruct) {
   ...
   //cout << "\n max font: width=" << char_max_width << " ascent=" << char_max_ascent << " descent=" << char_max_descent << std::flush;
...
unsigned int PopupX11::get_color_pixel(const RGBcolor & col) const {
  XColor xcol;
...
bool PopupX11::is_in_buttons_window(int x, int y) const {
...
void PopupX11::check_buttons_redraw(int highlight) {
...
void PopupX11::check_buttons_resize_window() {
  int s_x, s_y;
  compute_size_for_buttons_window(s_x, s_y);
  subwin.resize(s_x,s_y);
}

void PopupX11::get_y_bounds_of_label(int i, int & y_top, int & y_bottom) const {
  int dum= (char_max_descent + char_max_ascent + space_between_buttons);
  y_top= i * dum;
  y_bottom= y_top + dum;
}

int PopupX11::get_label_number_from_y_value(int y) const {
...
void __print_event_type(int type) {
  switch(type) {
  case KeyPress        : cout << "\n KeyPress        "; break;         
  case KeyRelease      : cout << "\n KeyRelease      "; break; 
  case ButtonPress     : cout << "\n ButtonPress     "; break; 
  case ButtonRelease   : cout << "\n ButtonRelease   "; break; 
  case MotionNotify    : cout << "\n MotionNotify    "; break; 
  case EnterNotify     : cout << "\n EnterNotify     "; break; 
  case LeaveNotify     : cout << "\n LeaveNotify     "; break; 
  case FocusIn         : cout << "\n FocusIn         "; break; 
  case FocusOut        : cout << "\n FocusOut        "; break; 
  case KeymapNotify    : cout << "\n KeymapNotify    "; break; 
  case Expose          : cout << "\n Expose          "; break; 
  case GraphicsExpose  : cout << "\n GraphicsExpose  "; break; 
  case NoExpose        : cout << "\n NoExpose        "; break; 
  case VisibilityNotify: cout << "\n VisibilityNotify"; break; 
  case CreateNotify    : cout << "\n CreateNotify    "; break; 
  case DestroyNotify   : cout << "\n DestroyNotify   "; break; 
  case UnmapNotify     : cout << "\n UnmapNotify     "; break; 
  case MapNotify       : cout << "\n MapNotify       "; break; 
  case MapRequest      : cout << "\n MapRequest      "; break; 
  case ReparentNotify  : cout << "\n ReparentNotify  "; break; 
  case ConfigureNotify : cout << "\n ConfigureNotify "; break; 
  case ConfigureRequest: cout << "\n ConfigureRequest"; break; 
  case GravityNotify   : cout << "\n GravityNotify   "; break; 
  case ResizeRequest   : cout << "\n ResizeRequest   "; break; 
  case CirculateNotify : cout << "\n CirculateNotify "; break; 
  case CirculateRequest: cout << "\n CirculateRequest"; break; 
  case PropertyNotify  : cout << "\n PropertyNotify  "; break; 
  case SelectionClear  : cout << "\n SelectionClear  "; break; 
  case SelectionRequest: cout << "\n SelectionRequest"; break; 
  case SelectionNotify : cout << "\n SelectionNotify "; break; 
  case ColormapNotify  : cout << "\n ColormapNotify  "; break; 
  case ClientMessage   : cout << "\n ClientMessage   "; break; 
  case MappingNotify   : cout << "\n MappingNotify   "; break; 
  default:               cout << "\n UNKNOWN EVENT";
  }
}
)

bool PopupX11::responsible_for_event(const XEvent & event) {
  ...
  DEBUG( __print_event_type(event.type); cout << "<- not responsible " << std::flush;);

  return false;
}

bool PopupX11::process_event(const XEvent & event) {
...
bool PopupX11::button_pressed(const XEvent & event, int & button, int & mouse_button) const {
  if (!subwin.exists || !subwin.visible)
    return false;
...
void PopupX11::redraw() {
  check_buttons_redraw();
}

void PopupX11::set_background_color(const RGBcolor & col) {
...
void PopupX11::set_foreground_color(const RGBcolor & col) {
  unsigned int pixel= get_color_pixel(col);
  subwin.fg_color= col;
...
void PopupX11::set_highlight_color(const RGBcolor & col) {
  unsigned int pixel= get_color_pixel(col);
...
bool PopupX11::set_number_of_buttons(int num) {
  if (num <= 0)
    return false;
...
bool PopupX11::set_button_label(int num, const char * lab) {
...
const char * PopupX11::get_button_label(int num) const {
  if (num < 0 || num >= num_buttons)
    return 0;
...
void PopupX11::set_popup_visible(int x, int y, int max_size_x, int max_size_y) {
