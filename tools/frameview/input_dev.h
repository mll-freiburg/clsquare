...
*/
struct InputEvent {
  static const int MOUSE_BUTTON_PRESSED= 1;
  static const int MOUSE_BUTTON_RELEASED= 2;
  static const int MOUSE_BUTTON_MOTION= 3;
...
class InputDevice {
 protected:
  void add_fd_to_set(int fd, fd_set * set) { FD_SET(fd,set); }
  bool is_fd_in_set(int fd, fd_set * set) { return FD_ISSET(fd,set); }
 public:
  virtual ~InputDevice(){};
  ///use this method to set initial values
  virtual bool set_defaults() { return true; }

  virtual bool set_initial_area(const Area2d & area) { return false; }

  /** \short read options from a file 
      \param fname name of the file including options 
   */
  virtual bool process_options(const char * fname) { return true; }

  /** \short read options from array of strings
      \param argc number of entries
      \param argv double array of '\0' terminated strings
   */
  virtual bool process_options(int argc, char const* const* argv) { return true; }

  ///shows help for options provided by this device
  virtual void help_options(ostream & o) const { 
    o << "\nhelp not yet implementd"; 
  }
  virtual void help_char_command(ostream & o) const { 
    o << "\nhelp not yet implementd"; 
  }

  virtual void generate_file_options(ostream & o) const {
    o << "\n[not yet implemeted]"; 
  }
  /** \short process a keyboard event
      \return true, if a redraw is needed afterwards, false else.
  */
  virtual bool process_char_command(BuilderBase * build, MenuBase * menu, const InputEvent & event) { 
    return false; 
  }
  ...
  /** \short process a mouse button event
      \return true, if a redraw is needed afterwards, false else.
  */
  virtual bool process_mouse_button_event(BuilderBase * build, const InputEvent & event) {
    return false; 
  }
  ...
  /** \short process a menu button event
      \return true, if a redraw is needed afterwards, false else.
  */
  virtual bool process_menu_button(BuilderBase * build, MenuBase * menu, const InputEvent & event) { 
    return false; 
  }
  ...
  /** \short process a popup menu button event
      \return true, if a redraw is needed afterwards, false else.
  */
  virtual bool process_popup_button(BuilderBase * build, MenuBase * popup, const InputEvent & event) { 
    return false; 
  }

  virtual bool init_menu(MenuBase * menu) { 
    return false; 
  }

  virtual bool init_popup(MenuBase * popup) { 
    popup->set_number_of_buttons(1);
    popup->set_button_label(0,"empty");
    return false; 
  }

  virtual bool init_frames(BuilderBase * build) = 0;
  virtual bool init_connection() { return true; }

  virtual bool uses_mouse() const { return false; }
  virtual bool uses_popup() const { return false; }
  ...
  */
  virtual int set_fds(fd_set * set)= 0;
