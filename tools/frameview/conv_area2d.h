 ...
 */
#ifndef _CONV_AREA2D_H_
#define _CONV_AREA2D_H_

#include "area2d.h"

/** \short Implements conversion of areas under constraints.
 ...
 */
class ConvArea2d {
  /** \short indicates the level of changes. 
  ...
  */
  ...
  /** \short set new level with regard to old value of level
 ...
 public:
  ...
  bool needs_update() { return level != 0; }

  ///performs the update! returns true if some values of the area changed
  bool update();

  void set_win_size(int sx, int sy);

  void   set_min_size_x(double v) { min_size_x= v; use_min_size_x= true; update_level(3); }
  void unset_min_size_x()         { use_min_size_x= false; }

  void   set_min_size_y(double v) { min_size_y= v; use_min_size_y= true; update_level(3); }
  void unset_min_size_y()         { use_min_size_y= false; }
  
  void   set_max_size_x(double v) { max_size_x= v; use_max_size_x= true; update_level(2); }
  void unset_max_size_x()         { use_max_size_x= false; }

  void   set_max_size_y(double v) { max_size_y= v; use_max_size_y= true; update_level(2); }
  void unset_max_size_y()         { use_max_size_y= false; }

  void   set_min_pos_x(double v) { min_pos_x= v; use_min_pos_x= true; update_level(1); }
  void unset_min_pos_x()         { use_min_pos_x= false; }

  void   set_min_pos_y(double v) { min_pos_y= v; use_min_pos_y= true; update_level(1); }
  void unset_min_pos_y()         { use_min_pos_y= false; }
  
  void   set_max_pos_x(double v) { max_pos_x= v; use_max_pos_x= true; update_level(1); }
  void unset_max_pos_x()         { use_max_pos_x= false; }

  void   set_max_pos_y(double v) { max_pos_y= v; use_max_pos_y= true; update_level(1); }
  void unset_max_pos_y()         { use_max_pos_y= false; }
  
  /** \short sets a new area
      because this area can be arbitrary, you have to check all levels
   */
  void set_area(const Area2d & a) { area= a; update_level(4); }
  void get_area(Area2d & a) const { a= area; }
  ...
  */
  ...
  void change_area_using_subwin(int origin_x, int origin_y, int dest_x, int dest_y);

  void change_area_using_win_pos(int origin_x, int origin_y);
  ...
  Point2d get_point_from_win_pos(int x, int y) const;
