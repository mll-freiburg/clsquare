...
class DrawFrame {
  ...
  friend ostream& operator<< (ostream&,const DrawFrame&);
  //  friend main();
 ...
 public:
  ...
  void actualize(const Frame2d & p_frame, bool);
  void draw();
  void draw(DisplayBase * disp, const Area2d & area, const Frame2d & p_frame, bool chg);
};

class DrawFrameMap {
  friend ostream& operator<< (ostream&,const DrawFrameMap&);
  class Item {
  public:
    Item(DrawFrame*);
    int key;
  ...
  Item *list;
  static const int MAX_DIRECT= 100; // frames with key in (0..MAX_DIRECT) are mapped in O(1)
...
class DrawTree {
  friend ostream& operator<< (ostream&,const DrawTree&);
  DrawFrame *origin;
  DrawFrameMap frameMap;
 public:
  DrawTree();
  bool insert_in_frame(int fkey, DrawFrame *);
  bool insert_in_frame(int fkey, VisualObject2d *);

  bool remove_frame(int fkey);
  bool remove_in_frame(int fkey, int fo_key);
  bool empty_frame(int fkey);

  //Drehung um eigenen Ursprung um einen Winkel, alle Objekte und Unterframes 
  //drehen sich mit.
  bool rotate_frame(int fkey, const Angle&);
  //Translation in Bezug auf das eigene Koordinatensystem, alle Objekte und
  //Unterframes werden auch verschoben!
  bool translate_frame(int fkey,double, double);

  //Drehe ein Objekt oder ein UnterFrame (mit fo_key) um den Ursprung um einen Winkel
  bool rotate_in_frame(int fkey, int fo_key, const Angle&);
  //Translation 
  bool translate_in_frame(int fkey, int fo_key, double, double);

  bool set_object_color(int fkey, int okey,const RGBcolor &);

  bool set_position(int fkey,double,double);
  bool set_visible(int fkey,bool val);
  bool set_layer(int fkey,int val);
  //bool set_positon(int fkey,int ofkey,double,double);
  bool set_angle(int fkey,const Angle&);
  bool set_scale(int fkey,double scale);
  bool set_pos_ang(int fkey,double,double,const Angle&);
  void draw(const Area2d &);
  void draw(DisplayBase * disp, const Area2d & area);
};
#endif
